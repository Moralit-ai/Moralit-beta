...
/*module.exports = function(JSONObject, callback) {
...
console.log(JSONObject);
	...
	if(questionableAct){
		if(kill)
			callback(killingRes[Math.floor(Math.random() * 3)]);
		if(harm)
			callback(harmingRes[Math.floor(Math.random() * 3)]);
		if(adulter)
			callback(adulteryRes[Math.floor(Math.random() * 3)]);
		if(decieve)
			callback(deceptionRes[Math.floor(Math.random() * 3)]);
		return "1"
	}
