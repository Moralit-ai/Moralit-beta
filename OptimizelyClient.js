...
OptimizelyClient.prototype.updateExperiment = function(options) {
    ...
    //options.custom_css = options.custom_css || "";
    //options.custom_js = options.custom_js || "";
    options.audience_ids = options.audience_ids || "";
    options.percentageIncluded = options.percentageIncluded || "";
    if (!options.id) throw new Error("required: options.id");
    var theUrl = this.baseUrl + 'experiments/' + options.id;
    delete options.id
...
OptimizelyClient.prototype.getVariations = function(options) {
    ...
    if (!options.id) throw new Error("Required: options.id");
    var theUrl = this.baseUrl + 'experiments/' + options.id + '/variations';
    return rest.getAsync(theUrl, {
      method: 'get',
      headers: this.baseHeaders
    })
...
OptimizelyClient.prototype.createAudience = function(options) {
    var optionsToSend = {};
    options = options || {};
    if (!options.name) throw new Error("Required: options.edit_url");
    if (!options.id) throw new Error("Required: options.project_id");
...
/**
 * @public
 * @name  OptimizelyClient#createSchedule
 * @since  0.4.0
 * @description Creates a schedule for an experiment in Optimizely
 */
OptimizelyClient.prototype.createSchedule = function(options) {
    options = options || {};
    options.start_time = options.start_time || null;
    options.stop_time = options.stop_time || null;
    if(!options.start_time && !options.stop_time) throw new Error("required: start_time or stop_time");
    var postUrl = this.baseUrl + 'experiments/' + options.experiment_id +
      '/schedules/';
    delete options.experiment_id;
    return rest.postAsync(postUrl, {
 ...
 * @since  0.4.0
 * @description Gets the list of goals from a project
 */
OptimizelyClient.prototype.getGoals = function(options){
 ...
 /**
 * @public
 * @name  OptimizelyClient#putGoal
 * @since  0.4.0
 * @description Puts a goal in a project
 */ 
OptimizelyClient.prototype.putGoal = function(options) {
    options = options || {};
    options.id = options.id;
    options.experiment_ids = options.experiment_ids || [];
    if(!options.id) throw new Error('required: options.id');
    var putUrl = this.baseUrl + 'goals/' + options.id;
    delete options.id;
    return rest.putAsync(putUrl, {
      method: 'put',
      headers: this.baseHeaders,
...
////////////////
////// TODO : GET SCHEDULE / UPDATE SCHEDULE / DELETE SCHEDULE
////////////////
